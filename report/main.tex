...
Before evaluating our system, we established a baseline by testing 6 queries (3 English, 1 Bangla, 2 mixed) against Google, Bing, and DuckDuckGo.

\begin{table}[H]
\centering
\caption{Precision@1 for commercial search engines (6 queries).}
...
\bn{Dhakay shikkha} (Education in Dhaka, BN)    & 1.00 & 1.00 & 1.00 \\
...
Commercial engines achieve perfect Precision@1 on monolingual queries thanks to web-scale indexing and query understanding. However, they do \textit{not} perform true cross-lingual retrieval (e.g., an English query returning Bangla articles from our local corpus). This is the gap our system fills.
...
We evaluate all four retrieval methods using standard IR metrics. Since our hybrid model retrieves 10 results per query and we have 23 labelled queries, we compute Precision@10 (fraction of top-10 that are relevant), nDCG@10 (normalised discounted cumulative gain), and MRR (mean reciprocal rank of the first relevant result) based on manual relevance assessments of the retrieved documents.

\begin{table}[H]
\centering
\caption{Evaluation summary: average top-1 hybrid score and cross-lingual retrieval rate.}
...
Queries with hybrid score $\geq$ 90\%      & 12/23 (52.2\%) \\
Queries with hybrid score $<$ 80\%         & 1/23 (4.3\%) \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    ...
    \item \textbf{Hybrid ranking yields the best overall performance:} The weighted combination (BM25=0.2, Semantic=0.7, Fuzzy=0.1) achieves a mean top-1 score of 87.9\%, with 95.7\% of queries scoring $\geq$80\%. The hybrid model rescued 3 queries where BM25 returned zero results.

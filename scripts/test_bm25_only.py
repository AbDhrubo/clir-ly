"""
Module C - BM25 Only Test Suite
Test BM25 retrieval method on 30 test queries (15 English, 15 Bangla)
"""
...
def test_bm25(bm25: BM25Search, queries: List[Dict]) -> Dict:
    """Test BM25 on all queries."""
    ...
    for i, q in enumerate(queries, 1):
        ...
        try:
            ...
            search_results = bm25.search(query, k=10)
...
def print_results_summary(results: Dict):
    ...
    if en_queries:
        ...
        print(f"  Min Time: {min(en_times):.1f}ms")
        print(f"  Max Time: {max(en_times):.1f}ms")
        print()
    ...
    if bn_queries:
        ...
        print(f"  Min Time: {min(bn_times):.1f}ms")
        print(f"  Max Time: {max(bn_times):.1f}ms")


def save_results_json(results: Dict, filepath: str = "results/bm25_test_results.json"):
...
def main():
    ...
    print("\n")
    print("╔" + "="*68 + "╗")
    print("║" + " "*20 + "MODULE C - BM25 Test Suite" + " "*22 + "║")
    print("╚" + "="*68 + "╝\n")
    ...
    # Step 4: Run tests
    results = test_bm25(bm25, queries)

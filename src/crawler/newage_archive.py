"""
New Age Archive-based Crawler.

Uses the date-based archive pages at /archive?date=YYYY-MM-DD to discover articles.
"""
...
class NewAgeArchiveCrawler:
    """
    Archive-based crawler for New Age.
    ...
    ARCHIVE_URL = "https://www.newagebd.net/archive"
    ...
    def get_archive_page(self, date_str: str) -> Optional[str]:
        ...
        """
        url = f"{self.ARCHIVE_URL}?date={date_str}"
        try:
    ...
    def extract_article_urls(self, html: str) -> List[str]:
        ...
        for link in soup.find_all("a", href=True):
            href = link["href"]
            # Match article URLs - New Age uses /post/category/ID/title format
            if re.match(r"^/post/[^/]+/\d+", href):
                url = self.BASE_URL + href
                urls.append(url)
            elif re.match(r"^https?://www\.newagebd\.net/post/[^/]+/\d+", href):
                urls.append(href)
    ...
    def crawl(self, limit: int = 500, days_back: int = 60) -> int:
        """
...
def crawl_newage_archive(limit: int = 500, days_back: int = 60) -> int:
    """
    Convenience function to crawl New Age archive.
    """
    crawler = NewAgeArchiveCrawler()
